...
/*   Updated: 2024/04/08 17:47:27 by ymeziane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static mlx_image_t	*draw_player(mlx_t *mlx, t_point player)
{
	...
	if (!img || (mlx_image_to_window(mlx, img, player.x * TILE_SIZE + TILE_SIZE / 3, player.y * TILE_SIZE + + TILE_SIZE / 3) == -1))
		return (printf("Error\n"), NULL);
...
static mlx_image_t	*draw_void(mlx_t *mlx, t_point void_p)
{
	...
	img = mlx_new_image(mlx, TILE_SIZE, TILE_SIZE);
	if (!img || (mlx_image_to_window(mlx, img, void_p.x * TILE_SIZE - 1, void_p.y
				* TILE_SIZE - 1) == -1))
		return (printf("Error\n"), NULL);
	color_img(img, color, TILE_SIZE - 1, TILE_SIZE - 1);
	img->instances[0].z = 0;
...
int	draw_map_2D(t_game *game)
{
	...
	if (!draw_void(game->mlx, game->s_map.player))
		return (0);
	y = 0;
