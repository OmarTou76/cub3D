...
static void	compute_distance_and_select_wall(t_game *game, t_wall *wall)
{
	...
	while (true)
	{
		if (game->s_map.map[r(y) / TILE_SIZE][r(x) / TILE_SIZE] == '1') // Ex [r(156) / 24][r(108) / 24] = [6][4]
		{
			...
			//    à  sqrt(pow(-76,284111, 2) + pow(-132,627426, 2)) = sqrt(5 819,265591 + 17 590,034127) = 132,627425
...
void	get_wall(t_game *game, t_wall *wall, float right_angle, int index)
{
	...
	wall->column_angle = right_angle + (index * FOV / col_nb); // Ex 60 + (1 * 60 / 645) = 60.093021
