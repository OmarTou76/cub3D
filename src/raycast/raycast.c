...
static uint32_t get_pixel_from_texture(t_wall wall, int y)
{
	...
	texture_index = ((int)(texture_y * wall.img->height) * wall.img->width) + (int)(texture_x * wall.img->width);
	...
	while (y < (int)game->img_view_3d->height)
	{
		if (y < y_start)
			mlx_put_pixel(game->img_view_3d, game->img_view_3d->width - index,
						  y, ft_pixel(game->colors->ceiling[0], game->colors->ceiling[1], game->colors->ceiling[2], 0xFF));
		else if (y > y_start + wall.height)
			mlx_put_pixel(game->img_view_3d, game->img_view_3d->width - index,
						  y, ft_pixel(game->colors->floor[0], game->colors->floor[1], game->colors->floor[2], 0xFF));
		else
		{
			color = get_pixel_from_texture(wall, y - y_start);
			if (!(wall.crack && (int)color == -256))
				mlx_put_pixel(game->img_view_3d, game->img_view_3d->width - index, y, color);
		}
		y++;
	}
}
